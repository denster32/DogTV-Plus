...
struct DogTV_Tests {
    ...
    // New test for dichromatic color mapping
    @Test("Dichromatic Color Mapping Pipeline")
    func testDichromaticColorMapping() async throws {
        ...
        guard let testTexture = device.makeTexture(descriptor: descriptor) else {
            #expect(false, "Failed to create test texture")
            return
        }
        let visualRenderer = try VisualRenderer()
        guard let outputTexture = visualRenderer.applyDichromaticColorMapping(testTexture) else {
            #expect(false, "applyDichromaticColorMapping should return a texture")
            return
        }
        #expect(outputTexture.width == 1 && outputTexture.height == 1, "Output texture should have same dimensions")
        print("✅ Dichromatic color mapping pipeline executed successfully")
    }

    // New test for dynamic contrast variation
    @Test("Dynamic Contrast Variation Test")
    func testDynamicContrastVariation() async throws {
        guard let device = MTLCreateSystemDefaultDevice() else {
            #expect(false, "Metal device should be available")
            return
        }
        ...
        // Default mapping
        let visualRenderer = try VisualRenderer()
        guard let defaultOutput = visualRenderer.applyDichromaticColorMapping(testTexture) else {
            #expect(false, "Default mapping should return texture")
            return
        }
        // Variation mapping: restfulSleep has lower contrastMultiplier
        visualRenderer.applyContentCategoryVisuals(category: .restfulSleep)
        guard let variationOutput = visualRenderer.applyDichromaticColorMapping(testTexture) else {
            #expect(false, "Variation mapping should return texture")
            return
        }
        // Validate outputs differ by instance
        #expect(defaultOutput !== variationOutput, "Dynamic contrast variation should produce distinct texture instances")
        print("✅ Dynamic contrast variation produces distinct output textures")
    }
