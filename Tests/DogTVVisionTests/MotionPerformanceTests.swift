...
class MotionPerformanceTests: XCTestCase {
    ...
    private let minFrameRate: Double = 20.0

    /// Maximum acceptable frame rate
    private let maxFrameRate: Double = 120.0

    /// Acceptable frame rate variance
    private let maxFrameRateVariance: Double = 5.0

    /// Motion blur reduction threshold
    private let motionBlurReductionThreshold: Float = 0.7
    ...
    func testFrameRateStabilityAcrossBreeds() {
        ...
        for breed in testBreeds {
            ...
            assertFrameRateResults(frameRateResults, for: breed)
        }
    }
    ...
    func testMotionBlurReductionEffectiveness() {
        ...
        for scenario in testScenarios {
            ...
            assertMotionBlurResults(blurReductionResults, for: scenario.name)
        }
    }
    ...
    func testRenderingPerformance() {
        ...
        for scenario in performanceScenarios {
            ...
            assertRenderingPerformance(renderingPerformance, for: scenario.name)
        }
    }
    ...
    private func assertFrameRateResults(_ results: FrameRateResults, for context: String) {
        XCTAssertGreaterThan(
            results.averageFrameRate,
            minFrameRate,
            "Frame rate too low for \(context)"
        )

        XCTAssertLessThan(
            results.averageFrameRate,
            maxFrameRate,
            "Frame rate too high for \(context)"
        )

        XCTAssertLessThan(
            results.frameRateVariance,
            maxFrameRateVariance,
            "Excessive frame rate fluctuation for \(context)"
        )
    }

    private func assertMotionBlurResults(_ results: MotionBlurResults, for context: String) {
        XCTAssertGreaterThan(
            results.blurReductionEffectiveness,
            motionBlurReductionThreshold,
            "Insufficient motion blur reduction for \(context)"
        )

        XCTAssertLessThan(
            results.artifactIntensity,
            0.3,
            "Excessive rendering artifacts for \(context)"
        )
    }

    private func assertRenderingPerformance(_ results: RenderingPerformanceResults, for context: String) {
        XCTAssertLessThan(
            results.averageRenderTime,
            0.016, // Less than 1/60th of a second
            "Rendering too slow for \(context)"
        )

        XCTAssertLessThan(
            results.memoryUsage,
            500_000_000, // 500 MB
            "Excessive memory usage for \(context)"
        )
    }
}
