...
public class ContentService: ObservableObject {
    ...
    private func transitionTo(scene: Scene) {
        ...
        print("Scene metadata: Audio file - \(scene.metadata.audioFile), Visual intensity - \(scene.metadata.visualIntensity)")
    }
}
...
public actor DefaultSceneLoader: SceneLoading {
    ...
    private func mockScenes() throws -> [Scene] {
        return [
            try Scene(name: "Ocean Calm", description: "Gentle waves on a sunny beach.", type: .ocean, metadata: try SceneMetadata(audioFile: "ocean_ambience", visualIntensity: 0.4, audioIntensity: 0.5, recommendedDuration: 1800), duration: 1800, isActive: true),
            try Scene(name: "Forest Whispers", description: "A peaceful walk through a lush forest.", type: .forest, metadata: try SceneMetadata(audioFile: "forest_ambience", visualIntensity: 0.3, audioIntensity: 0.4, recommendedDuration: 2400), duration: 2400, isActive: true),
            try Scene(name: "Firefly Dance", description: "A magical night with dancing fireflies.", type: .fireflies, metadata: try SceneMetadata(audioFile: "fireflies_ambience", visualIntensity: 0.6, audioIntensity: 0.3, recommendedDuration: 1200), duration: 1200, isActive: true),
            try Scene(name: "Rainy Day", description: "Cozy up to the sound of gentle rain.", type: .rain, metadata: try SceneMetadata(audioFile: "rain_ambience", visualIntensity: 0.2, audioIntensity: 0.6, recommendedDuration: 3600), duration: 3600, isActive: true),
            try Scene(name: "Golden Sunset", description: "Watch the sun dip below the horizon.", type: .sunset, metadata: try SceneMetadata(audioFile: "sunset_ambience", visualIntensity: 0.7, audioIntensity: 0.3, recommendedDuration: 1500), duration: 1500, isActive: true),
            try Scene(name: "Starry Night", description: "A clear night sky full of twinkling stars.", type: .stars, metadata: try SceneMetadata(audioFile: "stars_ambience", visualIntensity: 0.8, audioIntensity: 0.2, recommendedDuration: 4800), duration: 4800, isActive: true)
        ]
    }
