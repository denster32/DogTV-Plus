...
public class SettingsService: ObservableObject {
    ...
    public func saveSettings() {
        do {
            ...
            self.error = error
            Task {
                await analyticsService.trackEvent(.event(name: "settings_save_failed", attributes: [AnyEquatable(error.localizedDescription)]))
            }
        }
    }
    ...
    public func updateMainVolume(to volume: Float) {
        ...
        saveSettings()
        Task {
            await analyticsService.trackEvent(.event(name: "master_volume_updated", attributes: [AnyEquatable(String(volume))]))
        }
    }
    ...
    public func toggleNatureSounds(on: Bool) {
        ...
        saveSettings()
        Task {
            await analyticsService.trackEvent(.event(name: "nature_sounds_toggled", attributes: [AnyEquatable(String(on))]))
        }
    }
    ...
    public func updateColorScheme(to scheme: ColorScheme) {
        ...
        saveSettings()
        Task {
            await analyticsService.trackEvent(.event(name: "color_scheme_updated", attributes: [AnyEquatable(scheme.rawValue)]))
        }
    }
    ...
    public func toggleHighContrast(on: Bool) {
        ...
        saveSettings()
        Task {
            await analyticsService.trackEvent(.event(name: "high_contrast_toggled", attributes: [AnyEquatable(String(on))]))
        }
    }

    // MARK: - User Management

    /// User ID for tracking and analytics
